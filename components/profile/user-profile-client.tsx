...
export default function UserProfile() {
  ...
  useEffect(() => {
    ...
    const createdEvents = events.filter((event) => {
      // Fix: Check if createdEvents exists and is an array before using includes
      const userCreatedEvents = Array.isArray(currentUser.createdEvents) ? currentUser.createdEvents : []
      return event.creator.id === currentUser.id || userCreatedEvents.includes(event.id)
    })

    // Filter events joined by the user
    const joinedEvents = events.filter((event) => {
      // Fix: Check if joinedEvents exists and is an array before using includes
      const userJoinedEvents = Array.isArray(currentUser.joinedEvents) ? currentUser.joinedEvents : []
  ...
  return (
    <div>
      <div className="flex flex-col items-center py-4 mb-6">
        ...
        {currentUser.subscription && <PremiumBadge plan={currentUser.subscription.plan} className="ml-2" />}
          ...
          {userEvents.created.length > 0 ? (
            <div className="space-y-4">
              {userEvents.created.map((event) => (
                <div
                  key={event.id}
                  className="flex items-center p-3 bg-white rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow"
